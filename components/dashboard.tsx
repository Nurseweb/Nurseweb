...
import { LogOut, BookOpen, ClipboardCheck, Shield, ArrowRight, Clock, GraduationCap, Plus, Eye, EyeOff, ToggleLeft, ToggleRight, Trash2, Users, X, Pencil } from "lucide-react"
import Quiz from "./quiz"
import CourseViewer from "./course-viewer"

const BASE_PDF = "http://localhost:5000/pdfs"

const mockUser = { name: "Super Admin", email: "admin@medicare.com", role: "admin" }
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "20px", width: "100%", maxWidth: "440px", overflow: "hidden", boxShadow: "0 24px 64px rgba(13,43,51,0.15)", border: "1px solid #dce9ec" }}>
        ...
        <div style={{ padding: "32px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
            <div>
              <h3 style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "1.2rem", fontWeight: 600, color: "#0d2b33", margin: "0 0 3px" }}>Шинэ хэрэглэгч</h3>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {[
              ...
              <div key={key}>
                <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#3d6672", marginBottom: "6px", letterSpacing: "0.02em" }}>{label}</label>
                <input className="field-input" type={type} placeholder={placeholder} value={(form as any)[key]} onChange={e => setForm(prev => ({ ...prev, [key]: e.target.value }))} required />
              </div>
            ))}
            <div>
              <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#3d6672", marginBottom: "6px", letterSpacing: "0.02em" }}>Нууц үг</label>
              <div style={{ position: "relative" }}>
                <input className="field-input field-input-pr" type={showPw ? "text" : "password"} placeholder="••••••••" value={form.password} onChange={e => setForm(prev => ({ ...prev, password: e.target.value }))} required />
            ...
            <button type="submit" disabled={loading} style={{ marginTop: "4px", width: "100%", background: loading ? "#8aacb4" : "#0e7490", color: "#fff", border: "none", borderRadius: "9px", padding: "13px", fontFamily: "'DM Sans', sans-serif", fontSize: "0.9rem", fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", transition: "background 0.2s" }}>
    ...
    <>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
          ...
          <button onClick={() => setShowCreate(true)} style={{ display: "flex", alignItems: "center", gap: "7px", background: "#0e7490", color: "#fff", border: "none", borderRadius: "9px", padding: "10px 18px", fontFamily: "'DM Sans', sans-serif", fontSize: "0.85rem", fontWeight: 600, cursor: "pointer", transition: "background 0.2s" }} onMouseEnter={e => (e.currentTarget.style.background = "#0c6783")} onMouseLeave={e => (e.currentTarget.style.background = "#0e7490")}>
        ...
        <div style={{ background: "#fff", border: "1px solid #dce9ec", borderRadius: "16px", overflow: "hidden" }}>
          {loading ? (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {users.map((u, i) => (
                  <tr key={u.id} style={{ borderBottom: i < users.length - 1 ? "1px solid #e8f0f2" : "none", transition: "background 0.15s" }} onMouseEnter={e => (e.currentTarget.style.background = "#f8fbfc")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <div style={{ display: "flex", gap: "6px" }}>
                        <button onClick={() => setEditingUser(u)} title="Засах" style={{ background: "none", border: "1px solid #dce9ec", borderRadius: "7px", padding: "6px 8px", cursor: "pointer", display: "flex", alignItems: "center", color: "#517882", transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "#0e7490"; e.currentTarget.style.background = "rgba(14,116,144,0.05)"; e.currentTarget.style.color = "#0e7490" }} onMouseLeave={e => { e.currentTarget.style.borderColor = "#dce9ec"; e.currentTarget.style.background = "none"; e.currentTarget.style.color = "#517882" }}>
                          <Pencil size={14} />
                        </button>
                        <button onClick={() => handleToggle(u.id)} disabled={actionLoading === u.id + "_toggle"} title={u.is_active ? "Идэвхгүй болгох" : "Идэвхжүүлэх"} style={{ background: "none", border: "1px solid #dce9ec", borderRadius: "7px", padding: "6px 8px", cursor: "pointer", display: "flex", alignItems: "center", color: u.is_active ? "#0e7490" : "#8aacb4", transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "#0e7490"; e.currentTarget.style.background = "rgba(14,116,144,0.05)" }} onMouseLeave={e => { e.currentTarget.style.borderColor = "#dce9ec"; e.currentTarget.style.background = "none" }}>
                          {u.is_active ? <ToggleRight size={15} /> : <ToggleLeft size={15} />}
                        </button>
                        <button onClick={() => handleDelete(u.id, u.name)} disabled={actionLoading === u.id + "_delete"} title="Устгах" style={{ background: "none", border: "1px solid #dce9ec", borderRadius: "7px", padding: "6px 8px", cursor: "pointer", display: "flex", alignItems: "center", color: "#b91c1c", transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "#b91c1c"; e.currentTarget.style.background = "rgba(220,38,38,0.05)" }} onMouseLeave={e => { e.currentTarget.style.borderColor = "#dce9ec"; e.currentTarget.style.background = "none" }}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
...
export default function Dashboard({ user = mockUser, courses = mockCourses, logout, onStartQuiz }: DashboardProps) {
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafb", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <main style={{ maxWidth: "1080px", margin: "0 auto", padding: "40px 32px 80px" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #0d2b33 0%, #0e4a5c 100%)", borderRadius: "20px", padding: "40px 48px", marginBottom: "40px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative" }}>
            <span style={{ fontSize: "0.7rem", fontWeight: 600, color: "rgba(255,255,255,0.45)", letterSpacing: "0.12em", textTransform: "uppercase", display: "block", marginBottom: "12px" }}>NCLEX-RN · 2026 бэлтгэл</span>
            <h1 style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "clamp(1.6rem, 3vw, 2.2rem)", fontWeight: 600, color: "#fff", margin: "0 0 10px", lineHeight: 1.25, letterSpacing: "-0.01em" }}>Тавтай морилно уу, {user?.name}</h1>
            <p style={{ fontSize: "0.9rem", color: "rgba(255,255,255,0.55)", margin: 0, maxWidth: "480px", lineHeight: 1.7 }}>Хичээл үзэх, шалгалт өгөх, ахиц дэвшлээ хянах — бүгд нэг дор.</p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "4px", background: "#eef3f5", borderRadius: "12px", padding: "4px", width: "fit-content", marginBottom: "32px" }}>
          {tabs.map(({ id, label, icon: Icon }) => (
            <button key={id} className="tab-btn" onClick={() => { setActiveTab(id); setSelectedCourse(null) }} style={{ display: "flex", alignItems: "center", gap: "7px", padding: "9px 20px", borderRadius: "9px", fontSize: "0.84rem", fontWeight: 600, color: activeTab === id ? "#0d2b33" : "#7a9ea7", background: activeTab === id ? "#fff" : "transparent", boxShadow: activeTab === id ? "0 1px 6px rgba(14,116,144,0.08)" : "none" }}>
              <Icon size={15} />{label}
            </button>
          ))}
        </div>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "20px" }}>
                {courses.map((course, i) => (
                  <button key={course.id} className="course-card"
                    onMouseEnter={() => setHoveredCard(i)}
                    onMouseLeave={() => setHoveredCard(null)}
                    onClick={() => setSelectedCourse(course)}
                    style={{ borderRadius: "16px", overflow: "hidden", border: "1px solid #dce9ec" }}
                  >
                    <div style={{ aspectRatio: "16/8", overflow: "hidden", position: "relative", background: "#e0eef2" }}>
                      ...
                      <span style={{ position: "absolute", top: "12px", right: "12px", background: "#0e7490", color: "#fff", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "3px 10px", borderRadius: "99px" }}>
                        МН · EN
                      </span>
